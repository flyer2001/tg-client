...
1. **[DEV-1] Swift-DocC Documentation** 🔥🔥 - Организация документации (начать с авторизации, ~2 часа)
2. **[MVP-1.5] Типизация TDLib методов** 🔥 - Chat, Message, GetChats, GetChatHistory, ViewMessages
3. **[MVP-1.1-1.4] Реализация ChannelMessageSource** - MessageSource protocol + получение/отметка сообщений
...
**1.1 Инфраструктура (10 мин)**
- [ ] Создать `Sources/TgClient/TgClient.docc/` структуру
- [ ] Обновить `Package.swift` (добавить target `TgClient` с `.docc`)
- [ ] Создать скрипт `scripts/preview-docs.sh` для локального preview
- [ ] Проверить генерацию: `swift package preview-documentation --target TgClient`

**1.2 Главная страница (10 мин)**
- [ ] Создать `TgClient.md` (точка входа):
  - Описание проекта (CLI-клиент Telegram для AI-саммари)
  - Технологический стек (Swift 6, TDLib, Swift-Log, Swift Testing)
  - Статус реализации (✅ Реализовано, 🚧 В разработке, 📋 Запланировано)
  - Ссылки на E2E сценарии
  - Список core модулей MVP
  - Ссылки на внешние ресурсы (GitHub, TDLib docs)

**1.3 E2E: Авторизация (30 мин)**
- [ ] Создать `E2E-Scenarios/Authentication.md`:
  - Описание сценария (phone → code → ready)
  - Flow схема (включая 2FA)
  - Таблица обработки ошибок (PHONE_NUMBER_INVALID, etc.)
  - Ссылки на используемые компоненты (→ Component tests)
  - Ссылки на unit-level модели (→ Unit tests)
  - Ссылки на внешнюю документацию TDLib
...
- [ ] Добавить в [CONTRIBUTING.md](CONTRIBUTING.md):
  - **Правило:** При добавлении нового модуля → создать article в `Component-Tests/`
  - **Правило:** При добавлении E2E сценария → создать article в `E2E-Scenarios/`
  - **Правило:** При добавлении модели/запроса → обновить unit-тесты с doc comments
  - **Правило:** Перед коммитом → проверить `swift package preview-documentation`
- [ ] Обновить [CLAUDE.md](../CLAUDE.md):
  - Добавить в секцию "Перед коммитом": актуализировать DocC документацию
  - Ссылка на DEV-1 задачу

**1.8 Публикация документации (опционально, после MVP)**
- [ ] Настроить GitHub Actions для генерации статических страниц
