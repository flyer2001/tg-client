...
1. **[DEV-0.3] Тестирование удаленной разработки с iPhone** 🔥 - авторизация Claude CLI, проверка workflow
2. **[TEST-0.5a] GitHub Actions CI для Linux** 🔥 - автоматическая сборка и тесты на Linux
3. **[MVP-1.1] ChannelMessageSource: базовая структура** - протокол + TDLib реализация
...
- [ ] Авторизовать Claude CLI через `claude setup-token`
- [ ] Проверить работу Claude CLI на VPS с iPhone
- [ ] Попробовать выполнить простую задачу разработки с iPhone
- [ ] Документировать процесс в SETUP.md
