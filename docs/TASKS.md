...
1. **[5.1] Стратегия тестирования и покрытие кода** - TESTING.md, unit-тесты для моделей/encoder
2. **[3.7] Рефакторинг для тестируемости** - DI, протоколы, mock-объекты (требуется для component-тестов)
3. **[3.4] Добавить комментарии в TDLibAdapter** - документация классов и методов

---

## 📊 High Priority

### 5.1 Стратегия тестирования и покрытие кода

**Проблема:** Нет стратегии тестирования, текущий функционал не покрыт тестами.

**Цель:** Определить уровни тестирования, создать TESTING.md, покрыть существующий код.

**Задачи:**

#### Документация стратегии
- [ ] Создать `docs/TESTING.md` с описанием уровней тестирования:
  - **Unit-тесты:** изолированное тестирование функций/классов (моки для зависимостей)
  - **Component-тесты:** тестирование модулей с реальными зависимостями (но без сети)
  - **E2E-тесты:** интеграционные тесты с реальным TDLib/сетью (опционально, CI skip)
- [ ] Описать TDD workflow: RED → GREEN → REFACTOR
- [ ] Определить минимальный coverage для PR (например, 80%)
- [ ] Стратегия для async/await кода (Task.sleep vs ActorIsolated)

#### Покрытие существующего кода
**TDLibCodableModels (Unit-тесты):**
- [ ] `TDLibRequestEncoder`: кодирование всех Request моделей в JSON
- [ ] `TDLibUpdate`: декодирование различных типов ответов (error, authState, unknown)
- [ ] Response модели: декодирование из JSON примеров
- [ ] Request модели: проверка snake_case маппинга через CodingKeys

**TDLibAdapter (Component-тесты с моками):**
- [ ] `TDLibClient.send()`: корректная сериализация и вызов C API (мок td_json_client_send)
- [ ] `TDLibClient.receive()`: парсинг JSON от TDLib
- [ ] Авторизация: state machine переходы, обработка ошибок, timeout

**E2E-тесты (опционально):**
- [ ] Полный цикл авторизации с реальным TDLib (требует credentials, только локально)

**Инфраструктура:**
- [ ] Настроить `swift test` в CI
- [ ] Coverage отчёты (llvm-cov)
- [ ] Удобный запуск: `make test`, `make test-coverage`

**Зависимости:**
- Требует завершения 3.7 (DI/моки для component-тестов)
- Блокирует переход на полноценный TDD workflow
