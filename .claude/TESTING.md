...
E2E тесты **отключены по умолчанию** через `.disabled()` trait.

```bash
# Обычный запуск - БЕЗ E2E тестов (быстро)
swift test

# E2E тесты запускаются ВРУЧНУЮ из Xcode по необходимости
# (требуют credentials и авторизованную сессию в ~/.tdlib/)
```
...
#### Итеративный алгоритм Outside-In (как мы реально работаем)

**ВАЖНО:** Этот алгоритм показывает **реальный итеративный процесс**, а не линейный план.

```
1. Написать высокоуровневый Component Test
   └─> НЕ КОМПИЛИРУЕТСЯ (нет компонента)

2. Создать заглушки (минимум чтобы скомпилировалось)
   └─> Компилируется, падает на fatalError

3. ПОПЫТАТЬСЯ реализовать метод
   └─> Пишем код как будто всё есть
   └─> ⚠️ СТОП! Обнаружили: нужен tdlib.loadChats() — НЕ КОМПИЛИРУЕТСЯ

4. Добавить тест для недостающего функционала (В ТОМ ЖЕ файле!)
   └─> @Test("loadChats + updates stream")
   └─> НЕ КОМПИЛИРУЕТСЯ (нет loadChats/updates)
   └─> Это новый уровень декомпозиции

5. Спуститься на уровень ниже → Unit тесты для моделей
   └─> LoadChatsRequestTests, UpdateTests
   └─> Models implementation
   └─> Real implementation (loadChats в TDLibClient)
   └─> Mock implementation

6. Вернуться на уровень выше → попытка #2
   └─> Продолжить реализацию fetchUnreadMessages()
   └─> ⚠️ СТОП! Обнаружили: нужен tdlib.getChatHistory()

7. Повторить шаги 4-6 пока высокоуровневый тест не станет GREEN
```

**Ключевое отличие от классического Outside-In:**
- ❌ НЕ планируем все уровни заранее (overengineering)
- ✅ Обнаруживаем недостающее **при попытке реализации**
- ✅ Все тесты для разных уровней в **одном файле** (видна декомпозиция)

**Пример из проекта:** `ChannelMessageSourceTests.swift`
- Высокоуровневый тест `fetchUnreadMessages()`
- Секция `// MARK: - Тесты для недостающего функционала`
- Тест `loadChatsEmitsUpdateNewChat()` — обнаружен при попытке реализации

#### Порядок разработки новой фичи (классический Outside-In, для reference)
...
**Пример ошибки из проекта (MVP-1.6, 2025-11-12):**
```
❌ Предположили что ChannelMessageSource сложный БЕЗ попытки написать тест
❌ Создали UpdatesHandler (оказалось 3 строки = for await loop)
❌ Создали ChannelCache для realtime мониторинга (НЕ нужен для stateless MVP)
❌ Overengineering вместо простого решения

✅ Правильно было: попытаться написать Component Test сразу
✅ Оказалось что ChannelMessageSource вполне укладывается в 1 actor без декомпозиции
```
...
3. Декомпозиция (ARCHITECTURE DECISION)
   └─> Выделяем подкомпоненты по зонам ответственности:
       ├─ [Пример] ChannelCache (actor) — кэширование списка каналов
       ├─ [Пример] UpdatesHandler — обработка TDLib updates
...
4. Unit Tests для подкомпонентов (LOW-LEVEL)
   ├─ [Пример] ChannelCacheTests — изолированно
   ├─ [Пример] UpdatesHandlerTests — изолированно
...
> ⚠️ **ВАЖНО:** Это ГИПОТЕТИЧЕСКИЙ пример для демонстрации процесса декомпозиции.
> В реальности для MVP-1.6 ChannelMessageSource оказался достаточно простым (stateless подход),
> декомпозиция НЕ потребовалась. ChannelCache/UpdatesHandler были удалены как overengineering.
>
> **Правило остается:** декомпозируем ТОЛЬКО после реальной попытки написать Component Test,
> если видим > 50 строк кода и много аспектов ответственности.
>
> См. BACKLOG.md → Realtime мониторинг (коммиты `00c9f3f`, `1a6a762`) для контекста.
