...
- Правило явной индикации: 🤖 СУБАГЕНТ, 🎭 РОЛЬ, 🧠 МОДЕЛЬ

**TODO:**
- Протестировать субагентов на реальных задачах
- Создать дополнительные агенты (test-swift, security-ios)

**Ссылки:**
- HYP-001: Obsidian vault `_project-hub/hypotheses/active/HYP-001-subagents-perplexity.md`
...
**Выполнено:**
- ✅ Ретроспектива v0.4.0 завершена по процедуре RETRO-RULES.md (7 шагов)
- ✅ Анализ метрик: все 6 целевых метрик достигнуты (100%) 🎉
- ✅ Проход по 4 инцидентам → 5 новых правил для v0.5.0
- ✅ Итоги ретро записаны в retro-v0.4.0.md (append)
- ✅ Создан шаблон retro-v0.5.0.md (промпт + целевые метрики)
- ✅ RETRO-RULES.md создан (процедура проведения ретроспективы)
- ✅ TASKS.md обновлён: удалены завершённые задачи (DocC, ARCHITECTURE.md, Swift 6.2, Ретро v0.4.0)
- ✅ TASKS.md: добавлена секция "Подготовка к v0.5.0" (7 задач для обновления документации)
- ✅ Файлы ретро переименованы (убран суффикс -questions для единообразия)
- ✅ retro-v0.3.0-questions.md перемещён в архив
- ✅ DocC документация проверена (всё работает корректно)

**Изменённые файлы:**
- `.claude/archived/retro-v0.4.0.md` — итоги ретро v0.4.0 (append)
- `.claude/archived/retro-v0.5.0.md` — шаблон для отслеживания метрик v0.5.0
- `.claude/archived/RETRO-RULES.md` — процедура проведения ретроспективы (7 шагов, чек-лист)
- `.claude/TASKS.md` — удалены завершённые задачи, добавлена секция "Подготовка к v0.5.0"
- `.claude/archived/retro-v0.3.0-questions.md` — перемещён из .claude/ в .claude/archived/

**Решения/контекст:**

**Метрики v0.4.0 (все достигнуты):**
- Research-First: 100% ✅ (было 25% в v0.3.0)
- Mock только boundaries: 100% ✅ (было 0%)
- Дубликаты типов: 0% ✅ (было 16%)
- Code Review: 100% ✅ (новая метрика)
- TSan: 1 race condition найден ДО production ✅ (MockLogger)
- Concurrency: 0 рефакторингов ✅ (Architecture-First работает)

**Инциденты v0.4.0 → Новые правила v0.5.0:**
1. User Story пропущен → Planning Architect + Testing Architect gating (User Story + spike research ДО TDD)
2. TDLIB_DATABASE_ENCRYPTION_KEY → one-off инцидент, уже исправлено (.env.example обновлен)
3. Нарушение TDD порядка → Чек-лист "После spike search → вернуться к TDD" + Правило 0 (Grep/Read похожий код ПЕРЕД написанием)
4. Code Review пропущен → триггер в /endtask + /endsession ("Последняя на сегодня?" → промпт с Code Review)

**Метрики для отслеживания v0.5.0 (7 шт):**
- 3 критичных (Research-First, Mock boundaries, TSan) — оставлены под мониторинг
- 4 новых (Code Review триггер, User Story gating, Unit→Component порядок, Правило 0)
- 2 убраны (Дубликаты типов, Преждевременное завершение) — стали естественными, два релиза подряд 0%

**Задачи для v0.5.0 (ПЕРЕД началом версии):**
- ROLES.md — Planning Architect + Testing Architect gating
- CLAUDE.md — Правило 0 + Code Review триггер
- TESTING.md — чек-лист "После spike search"
- /endtask + /endsession — Code Review триггер

**Следующие шаги:**
1. Выполнить задачи "Подготовка к v0.5.0" (7 задач) ПЕРЕД началом новой версии
2. Регулярно проверять метрики v0.5.0 (раз в 1-3 дня, промпт в retro-v0.5.0.md)
...
- TESTING.md: критичные правила наверх (Research-First, Mock только boundaries)
- CLAUDE.md: дублирование критичных правил в 🔴 КРИТИЧНЫЕ WORKFLOW секции
- ROLES.md: новые роли (Senior Code Reviewer, Bugfix Specialist), расширен Senior Swift Architect (7 блоков)
- Начало отслеживания метрик v0.4.0 ([retro-v0.4.0-questions.md](archived/retro-v0.4.0-questions.md))

**Груминг v0.4.0: Mark as Read:**
- Architecture-First анализ (7 блоков): concurrency, производительность, отказоустойчивость, pipeline integration
- RFC создан: [MVP.md § v0.4.0](MVP.md#v040-mark-as-read)
  - Scope: TDLib viewMessages API, parallel mark-as-read (TaskGroup), CLI флаг
  - Архитектура: параллельное выполнение BotNotifier + MarkAsReadService (~2-5 сек ускорение)
  - Testing: Component тесты + TSan обязательно
  - Task Breakdown: 10 шагов TDD Outside-In
- Retry Strategy вынесена в BACKLOG (после v0.4.0): [BACKLOG.md#retry-strategy](BACKLOG.md#retry-strategy-best-practice)
- TASKS.md: актуализирован (удалены выполненные задачи, оставлен только контекст для v0.4.0)

**Следующая сессия:** DocC документация для MarkAsReadService ([TASKS.md § Задача #1](TASKS.md#1-реализация-v040-mark-as-read))

## [2025-12-11] - Завершение ретроспективы v0.3.0 + подготовка к v0.4.0

### Выполнено
- ✅ Задача #1 "Подготовка к v0.4.0: Усиление процессов" полностью завершена
- TESTING.md: критичные правила (Research-First, Mock только boundaries) в первых 20 строках + чеклисты External API
- CLAUDE.md: новая секция "🔴 КРИТИЧНЫЕ WORKFLOW" (5 подсекций) + code review в начале сессии
- ROLES.md: 2 новые роли (Senior Code Reviewer, Bugfix Specialist) + расширение Senior Swift Architect (7 блоков)
- BACKLOG.md: TSan учения + Swift 6.2 concurrency флаги

### Ключевые выводы ретро v0.3.0
- Research-First критичен: без него 100% багов, с ним 0%
- Правила работают когда их читают → критичные правила в первые 30 строк

### Следующая задача
- Задача #2: Груминг v0.4.0 (mark-as-read)
- Модель: вернуть на Sonnet перед началом разработки

## [2025-12-11] - Ретроспектива v0.3.0 завершена

**Цель сессии:** Провести ретроспективу v0.3.0, зафиксировать инсайты, составить план действий на v0.4.0

**Выполнено:**
- ✅ Ретроспектива v0.3.0 завершена:
  - 5 гипотез из ретро 2024-11 проверены (1 подтверждена, 4 отложены до v0.4.0)
  - 6 инцидентов проанализированы с root cause analysis
  - План действий на v0.4.0 составлен (6 задач обновления процессов)
  - 2 новые роли: Senior Code Reviewer, Bugfix Specialist
  - Файлы: retro-v0.3.0-results.md (финализирован), retro-v0.4.0-questions.md (создан)

- ✅ CLAUDE.md усилен правилами из популярного промпта:
  - При правках кода: НЕ дублируй весь код (2-3 строки до/после изменения)
  - Обращайся как к эксперту (без объяснений базовых концепций)
  - Сначала план кратко, потом детали по запросу

- ✅ ROLES.md обновлён:
  - AI-Assisted Developer: добавлены принципы ответов (код без дублирования, к эксперту, план → детали)

- ✅ BACKLOG.md: добавлена задача "TSan учения" (v0.4.0 тех долг)

- ✅ TASKS.md актуализирован:
  - Задача "Ретроспектива v0.3.0" завершена и удалена
  - Задача "SwiftPM Issue #9441" обновлена: root cause найден, прилинкованы 2 PR (#9490, #9493)
  - Добавлена задача "Подготовка к v0.4.0" (6 подзадач обновления TESTING/CLAUDE/ROLES)

**Ключевые выводы v0.3.0:**
- **Research-First критичен:** снижает риск багов с 100% до 0%
- **40% времени на багфикс** (архивные каналы) → главная проблема v0.3.0
- **Правила работают когда их читают:** критичные правила переместить в первые 30 строк TESTING.md

**Метрики v0.4.0 (целевые):**
- Research-First: 100% (↑ с 25%)
- Mock только boundaries: 100% (↑ с 0%)
- Дубликаты типов: 0% (↓ с 16%)
- Code Review: 100% дней (новое)
- Concurrency: 0 рефакторингов (новое)
...
**SwiftPM Issue #9441 прогресс:**
- Root cause диагностирован: architectural problem с Swift Concurrency/llbuild bridging
- Решение предложено: wrap blocking `build(target:)` в dedicated thread
- 2 PR прилинкованы (автор rconnell9): #9490, #9493 (оба OPEN)
- Следующий шаг: мониторить PR → тестировать фикс когда merge

**TODO:**
- Выполнить задачу "Подготовка к v0.4.0" (обновить TESTING.md/CLAUDE.md/ROLES.md)
- Начать регулярное отслеживание метрик (промпт в retro-v0.4.0-questions.md)

**Ссылки:**
- [retro-v0.3.0-results.md](archived/retro-v0.3.0-results.md) - финальные результаты ретро
- [retro-v0.4.0-questions.md](archived/retro-v0.4.0-questions.md) - промпт для отслеживания метрик
- [SwiftPM Issue #9441](https://github.com/swiftlang/swift-package-manager/issues/9441)
