...
**Проблема**: На Linux-серверах (проверено на Ubuntu 24.04) команда `swift build` может зависать на этапе "Planning build". Нагрузка на CPU при этом минимальная (~0%).

**Симптомы**:
- `swift build` висит на "Planning build" без прогресса
- Происходит как для первой сборки, так и для повторных
- Даже если бинарник уже собран
- Сообщение: "Another instance of SwiftPM (PID: XXX) is already running..."

**Причина**: SwiftPM создаёт lock-файлы в `.build/`. Если процесс был убит некорректно (Ctrl+C, kill, разрыв SSH), lock остаётся.

### Решение: Скрипт для сборки

⚠️ **ВАЖНО (Swift 6.2 на Linux):** Инкрементальная сборка НЕ работает из-за проблем SwiftPM. Каждая сборка = полная пересборка (~50-60 сек).
...
- Очищает глобальный кеш SwiftPM (`swift package purge-cache`)
- Сбрасывает локальный .build (`swift package reset`)
- Пересобирает проект (~50-60 сек)

**Когда использовать:**
- ✅ Всегда (единственный рабочий вариант на Linux)
- ✅ После любых изменений в коде
- ✅ При запуске тестов

### Почему долгая сборка на Linux?

**SwiftPM 6.2 на Linux зависает** при инкрементальных сборках:
- Проблема: SQLite `build.db` с `busy_timeout=0` → deadlock при повторных сборках
- Попытки исправить: ❌ Удаление только `build.db`, ❌ `purge-cache` без `reset`
- Единственное решение: Полная пересборка через `purge-cache + reset`

**Время сборки:**
- macOS: инкрементальная сборка ~5-10 сек ✅
- Linux: полная пересборка ~50-60 сек каждый раз ⚠️

**Отслеживание проблемы:**
- Issue в Swift: [будет создан если проблема сохранится]
- Workaround: использовать `build-clean.sh` до исправления в SwiftPM

**TODO: Дополнительные варианты для проверки:**
- [ ] Исследовать зависание на компиляции плагина Swift-DocC Preview
  - Verbose логи показывают зависание именно на этом этапе
  - Возможно: отключить плагины Swift-DocC в Package.swift временно
  - Проверить: помогает ли сборка без плагинов для инкрементальности
...
Для предотвращения проблемы при разрыве SSH:

```bash
# Запустить сборку в tmux
tmux new-session -s build
./scripts/build-clean.sh

# Отключиться: Ctrl+B, затем D
# Подключиться: tmux attach -t build
```
...
### Вариант 1: Прямой запуск (разработка/отладка)

**Способ А: Запуск уже собранного бинарника (рекомендуется, мгновенный старт)**

```bash
# Экспортировать переменные из .env и запустить бинарник
export $(cat .env | xargs) && .build/debug/tg-client
```

**Способ Б: Через swift run (требует компиляции ~1-2 мин)**
...
**Примечания:**
- `.build/debug/tg-client` - debug сборка (создаётся через `./scripts/build-clean.sh`)
- `.build/release/tg-client` - release сборка (требует `swift build -c release`)
- `export $(cat .env | xargs)` - загружает все переменные из `.env` файла в текущую сессию
- Debug сборка содержит больше проверок и логирования, release - оптимизирована для производительности

**Логирование:**
```bash
# Перенаправить stderr в файл (ошибки + логи приложения)
...
CI **не запускается** при изменении только:
- `.claude/**` - вся папка с документацией
...
- **Build timeout**: 5 минут (для детекции зависаний SwiftPM)
