...
**Решение: Использовать скрипт чистой сборки** (рекомендуется):
```bash
# Скрипт автоматически убивает зависшие процессы и очищает кэш
./scripts/build-clean.sh
```

**Альтернатива: Очистка .build вручную**:
```bash
# Убить зависшие процессы Swift
pkill -9 swift

# Удалить .build и пересобрать (занимает ~30-40 секунд)
rm -rf .build && swift package resolve && swift build
```

**Рекомендация: Использование tmux** (для предотвращения проблемы):
```bash
