...
### Решение: Скрипты для сборки

Проект содержит два скрипта для работы с проблемой зависания:

#### 1. ИНКРЕМЕНТАЛЬНАЯ СБОРКА (для разработки, ~5-10 сек)
...
- **Сохраняет .build кэш** (не пересобирает зависимости)
...
#### 2. ЧИСТАЯ СБОРКА (при проблемах, ~40-50 сек)
...
**Что делает:**
- Убивает все Swift процессы
- **Удаляет .build полностью**
- Пересобирает все зависимости (SwiftLint ~30 сек)

**Когда использовать:**
- ⚠️ Странные ошибки компиляции
- ⚠️ После git pull с конфликтами
- ⚠️ После изменения Package.swift
- ⚠️ Перед важным коммитом (проверка чистой сборки)

### Почему долгая сборка на Linux?

**SwiftLint** + зависимости (swift-syntax, Yams, и др.) = ~10 модулей = ~30 секунд компиляции.

При `rm -rf .build` всё пересобирается с нуля.

**Решение:** Используй `build-incremental.sh` для разработки!
