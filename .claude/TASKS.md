...
**Проблема:**
MockTDLibClient дублирует логику Real TDLibClient (ResponseWaiters, updates loop) → при каждом рефакторинге Real клиента нужно вручную синхронизировать Mock. Это **анти-паттерн TDD**.

**Root cause:**
Мы создали параллельный функционал (MockClient) для написания тестов, вместо того чтобы мокать только точку взаимодействия с внешней системой (C API boundary).

**Решение (2 варианта):**
1. **Quick fix (для текущей сессии):** MockClient переиспользует ResponseWaiters из Real TDLibClient
2. **Правильное решение (follow-up):** Mock только C API boundary (`td_json_client_*` функции), Real TDLibClient используется в Component тестах

**Задачи:**

**1. Починить broken тесты** (~30 мин)
- [ ] ChannelMessageSourceTests: loadChats + updateNewChat
- [ ] ChannelMessageSourceTests: fetchUnreadMessages
- Синхронизировать MockClient с новой логикой TDLibClient (actor, TDLibJSON)

**2. Обновить TESTING.md - TDD Anti-Pattern** (~45 мин)
- [ ] Добавить секцию "❌ Anti-Pattern: Дублирование логики в Mock"
- [ ] Правило: **Mock только границу с внешней системой, НЕ создавай параллельную реализацию**
- [ ] Пример: MockTDLibClient дублирует ResponseWaiters → при рефакторинге ломаются тесты
- [ ] Правильный подход: Mock `td_json_client_*` C функции, используй Real TDLibClient в тестах
- [ ] Критерий проверки: "Если при рефакторинге Production кода ломаются Mock'и → Mock дублирует логику"

**3. Quick fix: переиспользовать ResponseWaiters** (~1 час)
- [ ] MockTDLibClient использует Real ResponseWaiters вместо собственной логики
- [ ] Убрать дублирование обработки updates stream
- [ ] Все Component тесты проходят

**4. (Optional) Рассмотреть полный рефакторинг** (~1 час, follow-up задача)
- [ ] Создать MockTDLibCClient (mock только C API: `td_json_client_receive`, `td_json_client_send`)
- [ ] TDLibClient принимает dependency injection для C client
- [ ] Component тесты используют Real TDLibClient + MockTDLibCClient

**Детали:** [RETROSPECTIVE.md](.claude/RETROSPECTIVE.md) секция #4

**Критерии успеха:**
- ✅ Все Component тесты проходят
- ✅ MockClient переиспользует части Real implementation (ResponseWaiters)
- ✅ TESTING.md содержит правило про anti-pattern дублирования Mock логики
- ✅ E2E проверка не регрессирует
