...
**Контекст последней сессии (2025-11-19 ночь):**
- ✅ **КРИТИЧНЫЙ РЕФАКТОРИНГ: TDLib Unified Background Loop (race condition fix)**
  - **Проблема:** `td_json_client_receive()` вызывался из 2х мест (auth loop + updates loop) → deadlock
  - **Решение:** Единый background loop + ResponseWaiters (NSLock) + CheckedContinuation
  - **Файлы:** TDLibClient.swift, TDLibClient+HighLevelAPI.swift, TDLibErrorResponse.swift
  - **Проверки на production TDLib:**
    - ✅ Authorization с существующей сессией: loadChats → 392 chats
    - ✅ Authorization с нуля: phone → code → 2FA → ready → 772 chats
    - ✅ Ошибка 404 корректно пробрасывается в waiter
    - ✅ Параллельные getChatHistory() работают
  - **Документация:** ADR-002 добавлен в ARCHITECTURE.md
  - **Известная проблема (НЕ критично):** continuation leaked при параллельных запросах одного типа
    - Причина: Dictionary перезаписывает continuation при `responseWaiters["messages"] = ...`
    - Решение (post-MVP): использовать `@extra` field для request_id маршрутизации
    - Не блокирует функциональность, минимальный memory leak

- 🚨 **TODO для СЛЕДУЮЩЕЙ сессии (КРИТИЧНО - БАГА!):**

  **ПРИОРИТЕТ 1: Исправить ResponseWaiters - перезатирание continuation при параллельных запросах**
  - **Проблема:** Dictionary перезатирает continuation → Tasks получают чужие данные или зависают навсегда
  - **Пример:** 5 параллельных getChatHistory() → только последний Task получит ответ, остальные зависнут
  - **Решение:** `[String: [CheckedContinuation]]` вместо `[String: CheckedContinuation]` (массив!)
  - **Детали:** Background loop должен resume ПЕРВЫЙ continuation из массива (FIFO)
  - **Тест:** Запустить 10 параллельных getChatHistory() и проверить что все получили свои данные
  - **Файлы:** `TDLibClient.swift` (ResponseWaiters class)

  **ПРИОРИТЕТ 2: Обновить ADR-002 в ARCHITECTURE.md**
  - Исправить раздел "Известные ограничения" - это НЕ "не критично", а **серьёзная бага**
  - Добавить описание проблемы перезатирания (потеря данных, зависшие Tasks)
  - Отразить решение через массив continuations

  **ПРИОРИТЕТ 3: Рефакторинг MockTDLibClient** (после фикса ResponseWaiters)
  - Переиспользовать исправленный ResponseWaiters из Real TDLibClient
  - Имитировать async message routing через continuation
  - Цель: тесты должны проверять параллельные запросы корректно
  - **Альтернатива:** Вынести ResponseWaiters в отдельный internal класс для share
