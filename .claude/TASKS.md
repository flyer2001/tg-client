...
## ⚠️ GIT SAFETY И ПРОЦЕДУРЫ РЕФАКТОРИНГА

### Безопасная работа с файлами

**Правила при перемещении файлов:**

1. **НИКОГДА не используй `cd`** во время git операций:
   - `cd` меняет working directory контекст
   - Используй absolute paths или оставайся в project root

2. **Всегда используй `git mv`** для tracked файлов:
   - Сохраняет git history
   - Автоматически stage'ит изменения

3. **Коммить промежуточные состояния** при больших рефакторингах:
   - После перемещения файлов → commit
   - После обновления импортов → commit
   - После каждого успешного этапа → commit

4. **Проверка перед деструктивными операциями:**
   - `git status` ВСЕГДА перед `git reset`/`git clean`
   - Показать пользователю ЧТО будет потеряно
   - Получить явное подтверждение
   - Предпочитать `git stash` вместо `git reset --hard`

**См. также:** CLAUDE.md → "Правила для ассистента" → "Безопасность работы с git"

### Процедура рефакторинга модулей (SwiftPM)

**Шаг 1: Планирование (в начале сессии)**
- [ ] Прочитать текущую структуру модулей из Package.swift
- [ ] Определить зависимости между модулями
- [ ] Спроектировать финальную структуру
- [ ] Получить одобрение пользователя

**Шаг 2: Обновление Package.swift**
- [ ] Создать новые targets в Package.swift
- [ ] Обновить dependencies между модулями
- [ ] Сделать commit: "build: add new module structure"

**Шаг 3: Создание директорий**
- [ ] Создать новые директории через `mkdir -p`
- [ ] Проверить создание через `ls -la`
- [ ] Сделать commit (если нужно): "build: create module directories"

**Шаг 4: Перемещение файлов (КРИТИЧНО!)**
- [ ] Использовать ТОЛЬКО absolute paths или оставаться в project root
- [ ] Для каждого файла: `git mv Sources/OldModule/File.swift Sources/NewModule/File.swift`
- [ ] НЕ использовать `cd` команды
- [ ] Проверять после каждых 3-5 файлов: `git status`
- [ ] Сделать commit после успешного перемещения: "refactor: move files to new modules"

**Шаг 5: Обновление импортов**
- [ ] Найти все файлы с устаревшими импортами: `grep -r "import OldModule"`
- [ ] Обновить импорты через Edit tool (массово)
- [ ] Проверить компиляцию: `swift build`
- [ ] Сделать commit: "refactor: update imports after module restructure"

**Шаг 6: Удаление @testable (если применимо)**
- [ ] Найти `@testable import` в TestHelpers: `grep -r "@testable import"`
- [ ] Убрать @testable для публичных API
- [ ] Проверить компиляцию
- [ ] Сделать commit: "test: remove @testable from public APIs"

**Шаг 7: Проверка**
- [ ] `swift build` успешно
- [ ] `swift test` все тесты проходят
- [ ] `git status` - нет неожиданных изменений
- [ ] Сделать final commit (если есть правки): "build: finalize module refactoring"

**⚠️ КРИТИЧНО:**
- Делай commits после КАЖДОГО шага (не батч в конце!)
- Если ошибка → НЕ делай `git reset --hard` → спроси пользователя
- Используй `git stash` для временного отката (можно восстановить)
...
**Выполнено:**
- ✅ Создан протокол `TDLibFFI` для мокирования C API boundary
- ✅ Реализованы `CTDLibFFI` (real) и `MockTDLibFFI` (mock)
- ✅ Рефакторинг TDLibClient: DispatchQueue для блокирующих операций receive()
- ✅ AsyncStream bridge между DispatchQueue и async/await Task
- ✅ Dependency Injection: два init метода (public + internal для тестов)
- ✅ Исправлены Response модели: добавлен `case type = "@type"` в CodingKeys
- ✅ Все Unit-тесты TDLibClient проходят успешно (3 теста)
- ✅ Документирован anti-pattern "invented @extra requirement" (RETROSPECTIVE.md #13)

**Детали:**
- Real TDLibClient логика (ResponseWaiters, JSON парсинг, error handling) теперь тестируется напрямую
- MockTDLibFFI предоставляет FIFO responses БЕЗ дублирования бизнес-логики
- Блокирующие td_json_client_receive() вызовы выведены на DispatchQueue (thread pool safety)

---

### ✅ ЗАВЕРШЕНО: TDLibJSON + ResponseWaiters actor (2025-11-21)

**Выполнено:**
- ✅ Создан `TDLibJSON` struct - Sendable-safe обёртка над `[String: Any]`
- ✅ ResponseWaiters конвертирован в `actor` (убрали NSLock)
- ✅ Убрано ВСЁ использование `@unchecked Sendable` и `nonisolated(unsafe)`
- ✅ Все unit-тесты ResponseWaitersTests используют AsyncStream + callback паттерн
- ✅ E2E проверка успешна (параллельные запросы работают)
- ✅ Исправлена бага: TDLib возвращает `isChannel` как Int (0/1), а не Bool
- ✅ Обновлена документация: RETROSPECTIVE.md (#4, #12), TESTING.md (actor тестирование)
