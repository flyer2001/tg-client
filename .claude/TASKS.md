...
> **Последнее обновление:** 2025-12-06 (сессия завершена)
> **Текущая версия:** v0.3.0 (готовится к релизу)
...
- [x] Все тесты GREEN (146/146 passed)
- [ ] **Запуск E2E теста с реальным OpenAI API** (требует OPENAI_API_KEY)
- [ ] **Проверка всех тестов на macOS** (после установки Swift 6.0)

### Интеграция с реальными данными
- [ ] **Запуск на боевом TDLib клиенте** (авторизация, реальные каналы)
- [ ] **Генерация первого дайджеста** (MessageSource → DigestOrchestrator)
- [ ] **Проверка OpenAI API лимитов:**
  - [ ] Rate limits (RPM/TPM) — не превышаем
  - [ ] Стоимость генерации (~$0.006 для 100 сообщений)
  - [ ] Длина ответа ≤ 3800 chars (резерв для Telegram 4096)
- [ ] **Проверка краевых сценариев:**
  - [ ] Пустой массив сообщений → пустой дайджест
  - [ ] Много сообщений (>100) → не ломается
  - [ ] Нерусские символы (emoji, Markdown) → корректно обрабатываются

### Документация
- [x] CHANGELOG.md — v0.3.0 описан (сессия 2025-12-05)
- [ ] **MVP.md** — обновить "Текущая версия: 0.3.0"
- [ ] **CLAUDE.md** — убрать упоминания обязательности build-clean.sh (Swift 6.0 fix)
...
- [ ] **Ретроспектива** (следующая 2025-12-08) — оценить гипотезы
