...
## Research-First для External APIs [RETRO-2024-11]

**⚠️ ОБЯЗАТЕЛЬНО при работе с новым внешним API** (TDLib метод, HTTP API, библиотека):

| Шаг | Действие | Результат |
|-----|----------|-----------|
| 1. Spike | WebFetch документации, минимальный тест (≤30 мин) | "API работает так: ..." |
| 2. Architecture | Как интегрируется? Edge cases? Concurrency? | Проговорить вслух |
| 3. ADR | Если >50 строк — записать в ARCHITECTURE.md | Документ решения |
| 4. TDD | Только после шагов 1-3 | RED → GREEN → REFACTOR |

**Причина:** Пропуск spike → 9 дней cascade changes (Race Condition инцидент).
