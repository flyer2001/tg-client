...
@Suite("E2E: Генерация AI-дайджеста")
    ...
    func generateSummaryThroughOpenAI() async throws {
        ...
        let messages = [
            SourceMessage(
                ...
                messageId: 100,
                content: "Вышел Swift 6.0 с новыми возможностями concurrency и улучшенной производительностью.",
                channelTitle: "Swift Разработчики",
                link: "https://t.me/swiftdev/100"
            ),
            SourceMessage(
                chatId: -1001234567890,
                messageId: 101,
                content: "Туториал: создание CLI-инструментов с ArgumentParser и async/await.",
                channelTitle: "Swift Разработчики",
            ...
            SourceMessage(
                ...
                messageId: 200,
                content: "Обновление для команды: изменены правила code review.",
                channelTitle: "Команда Проекта",
                link: nil
            )
        ]
        ...
        guard let apiKey = ProcessInfo.processInfo.environment["OPENAI_API_KEY"],
              !apiKey.isEmpty else {
            Issue.record("❌ OPENAI_API_KEY не найден в environment. Добавьте ключ в .env файл.")
            return
        }
        ...
        #expect(!summary.isEmpty, "Дайджест не должен быть пустым")
        #expect(summary.count <= 4096, "Дайджест должен соблюдать лимит Telegram (4096 символов)")

        // 7. Проверка группировки по каналам
        // Дайджест должен содержать заголовки обоих каналов
        #expect(summary.contains("Swift Разработчики"), "Должен содержать заголовок публичного канала")
        ...
        #expect(!summary.contains("-1009876543210"), "Не должно быть chatId приватного канала в summary")

        print("✅ Generated summary (\(summary.count) chars):")
        print(summary)
        print("\n📊 Token usage будет залогирован в console (см. логи SummaryGenerator)")
    }
}
